"""
Unified Storage Manager for the Claims Pipeline (SAFE + CONSISTENT).

Always returns consistent JSON structures:
data/
 └── claims/
        <claim_id>/
            raw/
...
def save_raw_upload(claim_id: str, file_name: str, file_bytes: bytes) -> str:
    claim_raw_dir = os.path.join(BASE_DIR, claim_id, "raw")
    _ensure_dir(claim_raw_dir)

    file_path = os.path.join(claim_raw_dir, file_name)
    ...
    _write_json(os.path.join(claim_raw_dir, f"{file_name}.json"), meta)
    return file_path
...
def save_extracted_text(claim_id: str, extracted: Dict[str, Any]) -> str:
    ...
    - ocr output
    - email extract output
    """
    out_dir = os.path.join(BASE_DIR, claim_id, "extracted")
    _ensure_dir(out_dir)
    ...
    save_path = os.path.join(out_dir, "text.json")
    _write_json(save_path, payload)
    return save_path


# -----------------------------------------------------
# 3. STRUCTURED FIELDS STORAGE
# -----------------------------------------------------
def save_structured_fields(
    ...
    ai_inferred_fields: Optional[Dict[str, Any]] = None,
) -> str:

    out_dir = os.path.join(BASE_DIR, claim_id, "fields")
    _ensure_dir(out_dir)
    ...
    payload = {
        ...
        "ai_inferred_fields": ai_inferred_fields or {},
    }

    save_path = os.path.join(out_dir, "fields.json")
    _write_json(save_path, payload)
    return save_path
...
def save_decision_result(claim_id: str, decision: Dict[str, Any]) -> str:
    out_dir = os.path.join(BASE_DIR, claim_id, "decision")
    _ensure_dir(out_dir)
    ...
    save_path = os.path.join(out_dir, "decision.json")
    _write_json(save_path, payload)
    return save_path


# -----------------------------------------------------
# 5. Load all claim data (for debugging / UI)
# -----------------------------------------------------
def load_claim(claim_id: str) -> Dict[str, Any]:
    ...
    return {
        "raw": load_json(os.path.join(base, "raw")),
        "extracted": load_json(os.path.join(base, "extracted", "text.json")),
        "fields": load_json(os.path.join(base, "fields", "fields.json")),
        "decision": load_json(os.path.join(base, "decision", "decision.json")),
    }


# -----------------------------------------------------
# StorageManager class (wrapper for pipeline_runner)
# -----------------------------------------------------
class StorageManager:
    def save_raw_upload(self, claim_id: str, file_path: str, uploader: str = ""):
        ...
        with open(file_path, "rb") as f:
            data = f.read()
        return save_raw_upload(claim_id, file_name, data)

    def save_extracted_text(self, claim_id: str, extracted_text: Dict[str, Any]):
        return save_extracted_text(claim_id, extracted_text)

    def save_structured_fields(self, claim_id: str, fields_bundle: Dict[str, Any]):
        return save_structured_fields(
            claim_id,
            raw_fields=fields_bundle.get("raw_fields", {}),
            normalized_fields=fields_bundle.get("normalized", {}),
            validation=fields_bundle.get("validation", {}),
            ai_inferred_fields=fields_bundle.get("ai_inference", {}),
        )

    def save_decision(self, claim_id: str, decision: Dict[str, Any]):
        return save_decision_result(claim_id, decision)

    def save_hitl_task(self, claim_id: str, task: Dict[str, Any]):
        path = os.path.join(BASE_DIR, claim_id, "hitl")
        _ensure_dir(path)
        payload = {"timestamp": _timestamp(), "task": task}
        file_path = os.path.join(path, "hitl.json")
